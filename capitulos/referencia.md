...
> El *event loop* no se destruye al terminar la función ***main()***, sino que si quedan tareas ejecutándose en segundo plano, se esperará (en segundo plano) a que terminen todas para destruir el *loop*. Ello es necesario por si esas tareas deben ejecutar código de limpieza (cierre de recursos, etc.).
