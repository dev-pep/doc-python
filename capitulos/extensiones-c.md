...
Otra forma de extraer objetos de una secuencia (lista, tupla,...) es mediante apuntadores en la lista de argumentos que pasarán a referenciar el objeto extraído. Es el caso de funciones como `PyArg_ParseTuple()`, `PyArg_UnpackTuple()`, entre muchas otras. En este caso, los argumentos que les pasamos que sean del tipo apuntador a `PyObject*` recibirán una referencia a un objeto *Python*. Es otra forma de recibir múltiples objetos por parte de una función. Es frecuente que estas funciones retornen una *borrowed reference*. Esto es así porque sigue existiendo un propietario de dichos objetos: el objeto secuencia (y por tanto el propietario o propietarios de tal objeto secuencia). Así, mientras se está ejecutando la función que ha recibido estas *borrowed references* podemos usar los objetos tranquilamente, ya que tales propietarios no se están ejecutando, es decir no decrementarán el conteo de estos objetos que hemos recibido.

En relación a esto, un error frecuente que puede cometer una función es el siguiente: la función llama a una de esta funciones que extrae un objeto de una lista retornando una *borrowed reference*. Pero lo trata como si tuviese la propiedad de dicha referencia, olvidándose de incrementar su conteo. En este caso, podría guardar tal referencia en una variable estática o global y acabaría retornando en algún momento. Más tarde, podría volver a ejecutarse, todavía manteniendo en esa variable global o estática una referencia a ese objeto. El problema es que entre llamadas, el propietario de la secuencia de la que se extrajo el objeto puede haber, por ejemplo, sacado al objeto de la lista, decrementando su conteo y posiblemente liberando el objeto. Entonces la función despistada tendría una referencia a un elemento liberado.

Por lo tanto, cuando usemos ese tipo de funciones que extrae elementos, o una función que retorna un objeto, pero en todo caso obteniendo *borrowed references*, debemos recordar que solo las podemos almacenar en variables locales, es decir, se perderán una vez salgan del *scope* de la función. Si deseamos mantenerlas más allá, hay que invocar `Py_INCREF()`. Otra soución es extraer estos elementos utilizando funciones genéricas del *API* que nos retornan el objeto junto con la propiedad de la referencia a él. Es el caso de las funciones que empiezan por `PyObject_`, `PyNumber_`, `PySequence_` o `PyMapping_`, que incrementan automáticamente el conteo de la referencia obtenida para que el llamador lo decremente cuando no la use más.
